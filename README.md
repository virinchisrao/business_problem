# 🧾 ecommerce-sales-etl
**Scalable daily-batch ETL for multi-channel sales data (CSV + REST API).**

---

## ✅ What It Does
1. Reads **CSV files** dropped into `data/raw/`  
2. Calls **REST API** (optional) for mobile / web sales  
3. Cleans, validates, and quarantines bad rows  
4. Aggregates:
   - Total sales & revenue per channel  
   - Top 5 best-selling products  
5. Saves:
   - ✅ Clean rows → PostgreSQL table `sales.sales_cleaned`  
   - 📊 Daily report → `sales.sales_daily_report` (queryable)  
   - 💾 CSV snapshot → `output/daily_sales_report_<date>.csv`

---

## 🚀 Quick Start (CSV Only)

### 1️⃣ Clone the Repo
```bash
git clone https://github.com/<your-username>/ecommerce-sales-etl.git
cd ecommerce-sales-etl
2️⃣ Setup PostgreSQL

Create database:

createdb ecommerce_sales
```


Then run the init script:

```bash
psql -d ecommerce_sales -f sql/init.sql
```
3️⃣ Configure

Edit config.yaml to add your database credentials.

4️⃣ Install Dependencies
python -m venv venv
# Activate virtual environment
# Windows:
venv\Scripts\activate
# macOS/Linux:
source venv/bin/activate

pip install -r requirements.txt

5️⃣ Run the Pipeline

Drop CSV files named like online_sales_2025-11-06.csv into data/raw/ and run:

python src/main.py


Reports appear in output/; query tables in pgAdmin or psql.

🔌 Adding REST API Ingestion
Example Endpoint
GET https://api.example.com/v1/sales?date=2025-11-06
[
  {"product_id":"P101","product_name":"Mouse","quantity":20,"unit_price":25.0,"channel":"mobile"}
]

Add to config.yaml
api:
  url: "https://api.example.com/v1"
  key: "${API_KEY}"   # never hard-code keys

Set API Key

Windows PowerShell

$env:API_KEY="abc123"


Linux/macOS

export API_KEY=abc123


Then rerun:

python src/main.py


If no API is available, comment out or remove the api: section — the pipeline automatically falls back to CSV-only mode.

🧪 Run Tests
pip install pytest

# Windows
$env:PYTHONPATH="."; pytest tests -v

# Linux/macOS
PYTHONPATH="." pytest tests -v


✅ 5 unit tests validate ETL logic.

📁 Folder Map
data/raw/          ➜ drop CSV files here
data/quarantine/   ➜ bad rows (CSV + DB)
output/            ➜ daily report CSV
logs/              ➜ pipeline_<date>.log
sql/init.sql       ➜ creates Postgres schema
src/               ➜ ETL modules
tests/             ➜ pytest suite

🔍 Logs & Error Handling

INFO → Normal progress

ERROR → Logged + printed; pipeline stops (cron retries next day)

Bad rows go to:

Table → sales_quarantine

CSV → data/quarantine/quarantined_<date>.csv

🚢 Deployment Hints

Schedule daily run → Windows Task Scheduler / cron / Airflow

Containerize → add Dockerfile (example in wiki)

Cloud-ready → point config.yaml to RDS, Supabase, or any Postgres instance

🤖 AI Usage

70 % boilerplate generated by ChatGPT, including folder tree, SQL, and modules.
Human-refined: JSONB adapter, Windows paths, logging, and tests.
Every AI-generated block is tagged # AI-generated, human-adjusted in code.
Built in ±2 hours — extend freely!
